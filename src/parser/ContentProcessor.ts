...
const splitMarkdownTokens = (text: string) =>
  text.match(/(#{1,6})|(\*{1,3})|(_{1,3})|(`{1,3})|(-{1,3})|([!\[\]\(\)>])|(\w+)|(\s+)|(\S)/g);
...
export class ContentProcessor {
  ...
  private emitNewline(tokens: ContentToken[]): void {
  ...
  private processCodeContent(content: string, tokens: ContentToken[]): ContentToken[] {
  ...
  private processMathBuffer(content: string, tokens: ContentToken[]): ContentToken[] {
  ...
  private processTableContent(content: string, tokens: ContentToken[]): ContentToken[] {
    const newlineIdx = content.indexOf('\n');
    ...
    if (isTableRow(rowLine)) {
      ...
      if (rowLine) tokens.push(...this.processTextContent(rowLine));
      this.emitNewline(tokens);
      this.state.newlineFlag = true;
      this.state.nextTitleFlag = 0;
      this.state.nextBoldText = false;
  ...
  private processTextContent(content: string): ContentToken[] {
    const tokens: ContentToken[] = [];
  ...
  private processLineWithInlineMath(line: string, tokens: ContentToken[]): void {
    let remaining = line;
    while (remaining) {
